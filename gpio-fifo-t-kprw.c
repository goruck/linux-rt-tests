...
//printf("size1: %i size2: %i\n", fifoa.size, fifob.size);
  ...
  memset(&wordkw[0], 0, sizeof(wordkw));
  ...
  while (1) {
    ...
    if ((GET_GPIO(PI_CLOCK_IN) == PI_CLOCK_HI) && (flag == 0)) { // write/read keypad data
      if (ts_diff(&t, &tmark) > NEW_WORD_VALID) { // check for new word
        ...
        fifo_write(&(((struct fifos *) f)->fifo1), wordkr, MAX_BITS); // write current keypad word to FIFO
        cmd = getBinaryData(word,0,8);
        if (cmd == 0x11)
          strncpy(wordkw, idle, MAX_BITS); // avoid contention with a keypad responding to panel query
        else
          fifo_read(&(((struct fifos *) f)->fifo2), wordkw, MAX_BITS); // get a keypad command to send to panel
        if (pthread_mutex_lock(&mtx) != 0) { // avail global protection mutex
          perror("panel_io: can't lock mutex\n");
          exit(-1);
        }
        avail += 2; // number of words for message i/o thread to process
        ...
        memset(&wordkr[0], 0, sizeof(wordkr));
      }
...
static void * msg_io(void * f) {
  ...
  while (1) {
      ...
      while(avail > 0) {
        fifo_read(&(((struct fifos *) f)->fifo1), word, MAX_BITS); // get panel data to/from keypads
      ...
      //for (j = 0; j < 999999; ) j++; // waste time
      if (pthread_mutex_unlock(&mtx) != 0) { // unlock avail global mutex
        perror("msg_io: can't unlock mutex\n");
        exit(-1);
      }
      // get new keypad data for next time - for now just a test
      if (i == 125)	   strncpy(wordk, enter_prog, MAX_BITS);
      else if (i == 150) strncpy(wordk, idle, MAX_BITS);
      else if (i == 175) strncpy(wordk, exit_prog, MAX_BITS);
      else if (i == 200) strncpy(wordk, idle, MAX_BITS);
