...
static void * panel_io(void *f) {
  char word[MAX_BITS];
  int flag = 0, bit_cnt = 0, data_cnt = 0, i;
...
printf(" to dataFifo\n");*/
        ...
        //data_cnt++; // increment word counter
      }
      tmark = t;
      flag = 0;
      t.tv_nsec += INTERVAL;
...
int main(int argc, char **argv)
{
  ...
  void *thread_result;
  cpu_set_t cpuset_main, cpuset_io;

  // CPU(s) for main thread
   ...
   */
  /*if (sched_setaffinity(0, sizeof(cpuset_main), &cpuset_main) != 0) {
    perror("main sched_setaffinity");
    exit(-1);
  }

  if (sched_getaffinity(0, sizeof(cpuset_main), &cpuset_main) != 0) {
    perror("main thread sched_getaffinity");
    exit(-1);
  }

  printf("Set returned by sched_getaffinity_np() contained:\n");
    for (i = 0; i < CPU_SETSIZE; i++)
        if (CPU_ISSET(i, &cpuset_main))
            printf("CPU %d\n",i); */
  ...
  pthread_attr_setaffinity_np(&my_attr, sizeof(cpuset_io), &cpuset_io);
  ...
  res = pthread_create(&a_thread, &my_attr, panel_io, (void *) &dataFifo);
  if (res != 0) {
  ...
  // lock main thread to cpu(s) defined in cpuset_main
  if (sched_setaffinity(0, sizeof(cpuset_main), &cpuset_main) != 0) {
    perror("main sched_setaffinity");
    exit(-1);
  }

  // decode and print to output
  /*if ( (out_file = fopen ("data", "w")) == NULL )
    printf ("*** data could not be opened. \n" );*/

    while (1) {
      if (pthread_mutex_lock(&mtx) != 0) {
        perror("main: can't lock mutex\n");
        exit(-1);
      }
      while(avail == 0)
